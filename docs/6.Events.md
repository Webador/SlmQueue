...
The `EVENT_PROCESS` event also has a reference to the job instance.
...
$em->attach(WorkerEvent::EVENT_PROCESS, function(WorkerEvent $e) use ($logger) {
    $result = $e->getResult();
...
stop processing the queue. This is useful if you have something like [supervisor](7.WorkerManagement.md) automaticly
...
class JobTranslatorStrategy extends AbstractStrategy
{
    ...
    public function attach(EventManagerInterface $events)
    {
        $this->listeners[] = $events->attach(WorkerEvent::EVENT_PROCESS, array($this, 'onPreJobProc'), 1000);
        $this->listeners[] = $events->attach(WorkerEvent::EVENT_PROCESS, array($this, 'onPostJobProc'), -1000);
    }

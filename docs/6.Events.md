...
public function attach(EventManagerInterface $events)
{
    ...
    $this->listeners[] = $events->attach(
        AbstractWorkerEvent::EVENT_FINISH,
        array($this, 'onFinish')
    );
}
...
A worker strategy may report a state when th eworker exits. The strategy need to listen to 'AbstractWorkerEvent::EVENT_PROCESS_STATE' event. The AbstractStrategy implements a `onReportQueueState` method that takes the `$this->state` and returns it as appropiate. 
 ...
 * `AbstractWorkerEvent::EVENT_BOOTSTRAP` just before loop is entered
 * `AbstractWorkerEvent::EVENT_FINISH` just after the loop has exited
 * `AbstractWorkerEvent::EVENT_PROCESS_QUEUE` fetch job(s)
 * `AbstractWorkerEvent::EVENT_PROCESS_JOB` processes job(s)
 * `AbstractWorkerEvent::EVENT_PROCESS_IDLE` when the queue is empty
 * `AbstractWorkerEvent::EVENT_PROCESS_STATE` collects 'states' from strategies.
...
```php
$em->attach(AbstractWorkerEvent::EVENT_PROCESS_JOB, function(ProcessJobEvent $e) {
	$queue = $e->getQueue();
	$job   = $e->getJob();
});
```
...
$em->attach(AbstractWorkerEvent::EVENT_PROCESS_JOB, function(ProcessJobEvent $e) use ($logger) {
    $result = $e->getResult();
...
class JobTranslatorStrategy extends AbstractStrategy
{
    ...
    public function attach(EventManagerInterface $events)
    {
        $this->listeners[] = $events->attach(AbstractWorkerEvent::EVENT_PROCESS_JOB, array($this, 'onPreJobProc'), 1000);
        $this->listeners[] = $events->attach(AbstractWorkerEvent::EVENT_PROCESS_JOB, array($this, 'onPostJobProc'), -1000);
    }

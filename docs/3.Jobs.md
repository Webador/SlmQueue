...
Every job must implement the interface `SlmQueue\Job\JobInterface`. For ease of use, an abstract class `SlmQueue\Job
\AbstractJob` is provided to implement most of the interface methods. The method, `execute()`, must be
...
class MyController extends AbstractActionController
{
    public function fooAction()
    {
        ...
        $job = $this->jobManager->get('send-email');
...
this also give you restrictions than payload data *must* be serializable. Doctrin2 entities, references to 
